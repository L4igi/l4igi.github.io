        ...
        >
            ...
            >
                ...
                <AnimatePresence>
                    ...
                    {isLegalOpen && <LegalModal onClose={() => setIsLegalOpen(false)} theme={theme} />}
                </AnimatePresence>
                ...
                <div className={`flex-1 flex-col w-full h-full relative ${isModalOpen ? 'hidden md:flex' : 'flex'}`}>
                    ...
                    <div
                        ...
                        style={{
                            backgroundColor: theme.isDark ? 'rgba(15, 23, 42, 0.6)' : 'rgba(241, 245, 249, 0.6)',
                    ...
                    <div className="flex-1 bg-[var(--panel)] relative flex flex-col shadow-[0_-10px_30px_rgba(0,0,0,0.05)] z-10 transition-colors duration-700 overflow-hidden">
                                ...
                                {viewMode === 'GRID' ? (
                                    ...
                                    >
                                        {filteredProjects.map((project) => (
                                            <GameTile key={project.id} project={project} onClick={() => { setIsPressed(true); setTimeout(() => setIsPressed(false), 150); launchProject(project); }} onHover={() => handleHover(project)} isSelected={selectedId === project.id} isPressed={selectedId === project.id && isPressed} isFavorite={favorites.includes(project.id)} theme={theme} />
                                        ))}
                                    </motion.div>
                                ) : (
                                    <motion.div
                                    ...
                                    >
                                        {filteredProjects.map((project) => (
                                            <ListRow key={project.id} project={project} onClick={() => launchProject(project)} onHover={() => setHoveredId(project.id)} isSelected={selectedId === project.id || hoveredId === project.id} isFavorite={favorites.includes(project.id)} theme={theme} />
                                        ))}
                                    </motion.div>
                                )}
                            </AnimatePresence>
                        ...
                        <div className="absolute bottom-6 right-6 z-50">
                            <motion.button
                                ...
                                title="Toggle View"
                            >
                                {viewMode === 'GRID' ? <LayoutGrid size={24} /> : <ListIcon size={24} />}
                            </motion.button>
                        </div>
