...
export const AboutModal = ({ onClose, theme }: { onClose: () => void, theme: Theme }) => {
    ...
    return (
        <div className={`fixed inset-0 z-[200] flex items-center justify-center p-4 sm:p-6 bg-black/90 sm:bg-black/70 sm:backdrop-blur-md transition-opacity duration-300 ${isClosing ? 'opacity-0' : 'opacity-100'}`}>
            ...
            >
                {/* --- HERO HEADER (Visible Immediately) --- */}
                <div className="relative h-40 shrink-0 overflow-hidden" style={{ backgroundColor: theme.colors.accent }}>
                    ...
                    <div className="relative z-10 h-full px-8 flex items-center justify-between">
                        <div className="ml-32 sm:ml-40 mt-4 text-white drop-shadow-md">
                            ...
                            <motion.p
                                layoutId="modal-role"
                                className="opacity-90 font-bold text-sm sm:text-base flex items-center gap-2 uppercase tracking-wider"
                            >
                ...
                {/* --- FLOATING AVATAR --- */}
                {/* Container always visible, image fades in when ready */}
                <div className="absolute top-16 left-8 z-20">
                    ...
                    >
                        ...
                        <div className="absolute inset-0 flex items-center justify-center">
                            <span className="text-4xl font-black opacity-20" style={{ color: theme.colors.accent }}>LH</span>
                        </div>
                ...
                {/* --- MAIN BODY (Fades in after expansion) --- */}
                            ...
                            <motion.div variants={contentStagger} initial="hidden" animate="visible" className="space-y-2">
                                {TABS.map((tab) => (
                                    <motion.button
                                        ...
                                        className={`w-full flex items-center gap-4 p-3 rounded-xl font-bold text-sm relative overflow-hidden group`}
                                        style={{
                    ...
                    <div className="flex-1 p-6 sm:p-10 overflow-y-auto custom-scrollbar relative" style={{ backgroundColor: theme.colors.cardBg }}>
                        {isExpanded && (
                            <AnimatePresence mode="wait">
                                {activeTab === 'ID' && (
                                    <motion.div key="ID" variants={contentStagger} initial="hidden" animate="visible" exit={{ opacity: 0, y: 10 }} className="space-y-8 pt-4">
                                        <motion.div variants={itemVariants} className="space-y-2">
                                            ...
                                            <p className="text-base sm:text-lg font-medium leading-relaxed" style={{ color: theme.colors.text }}>{t('modal.quote_text')}</p>
                                        </motion.div>
                                        <motion.div variants={itemVariants} className="w-full h-px opacity-10" style={{ backgroundColor: theme.colors.text }}></motion.div>
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                            {[
                                                ...
                                                <motion.div variants={itemVariants} key={i} className="p-5 rounded-2xl border shadow-sm flex items-start gap-4 group" style={{ backgroundColor: theme.colors.primary, borderColor: theme.colors.secondary }}>
                                        ...
                                        <motion.div variants={itemVariants} className="p-6 rounded-2xl border relative mt-4" style={{ backgroundColor: theme.colors.primary, borderColor: theme.colors.secondary }}>
                                            ...
                                            <p className="text-lg font-bold italic relative z-10 text-center" style={{ color: theme.colors.text }}>"Coding for compliments. Also building cool stuff with Kotlin & Godot. Moving forward to the next iteration."</p>
                                        </motion.div>
                                    </motion.div>
                                )}

                                {activeTab === 'EXP' && (
                                    <motion.div key="EXP" variants={contentStagger} initial="hidden" animate="visible" exit={{ opacity: 0 }} className="pt-4">
                                ...
                                {activeTab === 'SKILLS' && (
                                    <motion.div key="SKILLS" variants={contentStagger} initial="hidden" animate="visible" exit={{ opacity: 0 }} className="pt-4">
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                            {SKILLS_DATA.map((stat, i) => (
                                                <motion.div variants={itemVariants} key={stat.l} className="p-6 rounded-2xl shadow-sm border" style={{ backgroundColor: theme.colors.primary, borderColor: theme.colors.secondary }}>
                                                    ...
                                                    <div className="h-3 rounded-full overflow-hidden" style={{ backgroundColor: theme.colors.cardBg }}>
                                                        <motion.div initial={{ width: 0 }} animate={{ width: `${stat.v}%` }} transition={{ duration: 1, delay: 0.2 + (i * 0.1), ease: "easeOut" }} className="h-full rounded-full" style={{ backgroundColor: theme.colors.accent }}></motion.div>
                                                    </div>
                                                </motion.div>
                                            ))}
                                        </div>
                                ...
                                {activeTab === 'LIKES' && (
                                    <motion.div key="LIKES" variants={contentStagger} initial="hidden" animate="visible" exit={{ opacity: 0 }} className="grid grid-cols-1 gap-4 pt-4">
                                        {LIKES_DATA.map((item) => (
