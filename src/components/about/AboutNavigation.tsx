...
export const AboutNavigation = ({
      ...
      if (variant === "desktop") {
        ...
        return (
          <motion.button
            ...
            whileTap={{ scale: 0.9 }}
            onClick={() => setActiveTab(tab.id as AboutTab)}
            className={`relative rounded-xl font-bold text-xs flex items-center gap-2 transition-colors px-2 py-2.5 justify-center border cursor-pointer ${
              isActive
                ? "text-white border-transparent"
                : "border-transparent hover:bg-black/5"
            }`}
          ...
          >
            ...
            <span className="relative z-10 flex items-center gap-2">
              {tab.icon}
              {/* Optional: Add labels on mobile if space permits, or keep icons only for compactness */}
  ...
  if (variant === "mobile") {
    return (
      <div
        // CHANGED: pt-16 -> py-4. Removed fixed height constraints to let it flow naturally.
        className="sm:hidden w-full px-4 py-4 border-b z-10 grid grid-cols-2 gap-2 shrink-0"
  ...
  return (
    <div
      // CHANGED: pt-24 -> pt-6. This removes the large gap at the top of the sidebar .
      className="hidden sm:flex w-64 p-4 pt-6 flex-col gap-2 border-r z-10"
      style={{
